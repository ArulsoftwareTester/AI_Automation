...
{
    [TestFixture]
    public class T28037138_InternetControlPanel : PageTest
    {
        ...
        {
            _test = TestInitialize.CreateTest(TestContext.CurrentContext.Test.Name, "Windows 365 Security Baseline - Internet Control Panel Test");
            _test.Info($"Test started at: {DateTime.Now:yyyy-MM-dd HH:mm:ss}");
            _test.Info("Test ID: 28037138");
            _test.Info("Category: Security Baseline Configuration");
        }

        [Test]
        public async Task Test_28037138_InternetControlPanel()
        {
            try
            {
                ...
                await securityBaseline.CreateNewProfile(
                                                        ...
                                                        "Prevent ignoring certificate errors",   // Parent dropdown name
                                                        "Enabled",                        // Parent dropdown value
                                                        "", // Child dropdown name
                                                        "",                        // Child dropdown value
                                                        "Windows Components > Internet Explorer > Internet Control Panel",  // Parent path
                                                        "Windows Components > Internet Explorer > Internet Control Panel"   // Child path
                                                    );    
            ...
            {
                var errorScreenshot = await ExtentReportHelper.CaptureScreenshot(Page, "Error_28037138");
