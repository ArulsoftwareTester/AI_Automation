...
{
    [TestFixture]
    public class Lps_27894475_BackupDirectory : PageTest
    {
        ...
        {
            _test = TestInitialize.CreateTest(TestContext.CurrentContext.Test.Name, "Security Baseline for Windows 10 and later - lps: Backup Directory");
            _test.Info($"Test started at: {DateTime.Now:yyyy-MM-dd HH:mm:ss}");
            _test.Info("Test ID: 27894475");
            _test.Info("Category: Security Baseline for Windows 10 and later - lps");
        }

        [Test]
        public async Task Test_27894475_Lps_BackupDirectory()
        {
            try
            {
                ...
                _test?.Info("Step 2: Create new Security Baseline profile with Backup Directory setting");
                ...
                _test?.Info("Step 4: Edit profile again to change setting to Backup to Active Directory");
                await securityBaseline.editNewCreatedPolicy(
                    Page,
                    "Endpoint security",
                    "Security baselines",
                ...
                _test?.Pass("Successfully edited Security Baseline Profile - changed setting to Backup to Active Directory");

                var editScreenshot2 = await ExtentReportHelper.CaptureScreenshot(Page, "Profile_Edited_BackupToActiveDirectory");
                if (!string.IsNullOrEmpty(editScreenshot2))
                {
                    _test?.Pass("Profile edit to Backup to Active Directory completed", MediaEntityBuilder.CreateScreenCaptureFromPath(editScreenshot2).Build());
                }
            ...
            {
                var errorScreenshot = await ExtentReportHelper.CaptureScreenshot(Page, "Error_27894475");
