...
{
    [TestFixture]
    public class Kerberos_31491594_PKInitHashAlgorithmSHA512 : PageTest
    {
        ...
        {
            _test = TestInitialize.CreateTest(TestContext.CurrentContext.Test.Name, "Security Baseline for Windows 10 and later - Kerberos: PK Init Hash Algorithm SHA512");
            _test.Info($"Test started at: {DateTime.Now:yyyy-MM-dd HH:mm:ss}");
            _test.Info("Test ID: 31491594");
            _test.Info("Category: Security Baseline for Windows 10 and later - Kerberos");
        }

        [Test]
        public async Task Test_31491594_Kerberos_PKInitHashAlgorithmSHA512()
        {
            try
            {
                ...
                _test?.Info("Step 2: Create new Security Baseline profile with PK Init Hash Algorithm SHA512 setting");
                ...
                _test?.Info("Step 5: Edit profile again to change setting to Audited");
                await securityBaseline.editNewCreatedPolicy(
                    Page,
                    "Endpoint security",
                    "Security baselines",
                ...
                _test?.Pass("Successfully edited Security Baseline Profile - changed setting to Audited");

                var editScreenshot3 = await ExtentReportHelper.CaptureScreenshot(Page, "Profile_Edited_Audited");
                if (!string.IsNullOrEmpty(editScreenshot3))
                {
                    _test?.Pass("Profile edit to Audited completed", MediaEntityBuilder.CreateScreenCaptureFromPath(editScreenshot3).Build());
                }
            ...
            {
                var errorScreenshot = await ExtentReportHelper.CaptureScreenshot(Page, "Error_31491594");
