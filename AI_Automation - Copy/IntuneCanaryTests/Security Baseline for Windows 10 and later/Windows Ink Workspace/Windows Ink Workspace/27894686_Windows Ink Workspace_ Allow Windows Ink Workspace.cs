...
{
    [TestFixture]
    public class WindowsInkWorkspace_27894686_AllowWindowsInkWorkspace : PageTest
    {
        ...
        {
            _test = TestInitialize.CreateTest(TestContext.CurrentContext.Test.Name, "Security Baseline for Windows 10 and later - Windows Ink Workspace: Allow Windows Ink Workspace");
            _test.Info($"Test started at: {DateTime.Now:yyyy-MM-dd HH:mm:ss}");
            _test.Info("Test ID: 27894686");
            _test.Info("Category: Security Baseline for Windows 10 and later - Windows Ink Workspace");
        }

        [Test]
        public async Task Test_27894686_WindowsInkWorkspace_AllowWindowsInkWorkspace()
        {
            try
            {
                ...
                _test?.Info("Step 2: Create new Security Baseline profile with Allow Windows Ink Workspace setting");
                ...
                _test?.Info("Step 4: Edit profile again to change setting to Enabled above lock");
                await securityBaseline.editNewCreatedPolicy(
                    Page,
                    "Endpoint security",
                    "Security baselines",
                ...
                _test?.Pass("Successfully edited Security Baseline Profile - changed setting to Enabled above lock");

                var editScreenshot2 = await ExtentReportHelper.CaptureScreenshot(Page, "Profile_Edited_EnabledAboveLock");
                if (!string.IsNullOrEmpty(editScreenshot2))
                {
                    _test?.Pass("Profile edit to Enabled above lock completed", MediaEntityBuilder.CreateScreenCaptureFromPath(editScreenshot2).Build());
                }
