...
{
    [TestFixture]
    public class UserRights_27998813_DenyRemoteDesktopServicesLogOn : PageTest
    {
        ...
        {
            _test = TestInitialize.CreateTest(TestContext.CurrentContext.Test.Name, "Security Baseline for Windows 10 and later - User Rights: Deny Remote Desktop Services Log On");
            _test.Info($"Test started at: {DateTime.Now:yyyy-MM-dd HH:mm:ss}");
            _test.Info("Test ID: 27998813");
            _test.Info("Category: Security Baseline for Windows 10 and later - User Rights");
        }

        [Test]
        public async Task Test_27998813_UserRights_DenyRemoteDesktopServicesLogOn()
        {
            try
            {
                ...
                _test?.Info("Step 2: Create new Security Baseline profile with Deny Remote Desktop Services Log On setting");
            ...
            {
                var errorScreenshot = await ExtentReportHelper.CaptureScreenshot(Page, "Error_27998813");
