...
{
    [TestFixture]
    public class T21790040_Microsoft_Office_2016_Allow_VBA_To_Load_Typelib_References_By_Path_From_Untrusted_Intranet_Locations_User : PageTest
    {
        ...
        {
            _test = TestInitialize.CreateTest(TestContext.CurrentContext.Test.Name, "Microsoft 365 Apps for Enterprise Security Baseline - Microsoft Office 2016 Allow VBA To Load Typelib References By Path From Untrusted Intranet Locations User Test");
            _test.Info($"Test started at: {DateTime.Now:yyyy-MM-dd HH:mm:ss}");
            _test.Info("Test ID: 21790040");
            _test.Info("Category: Security Baseline Configuration");
        }

        [Test]
        public async Task Test_21790040_Microsoft_Office_2016_Allow_VBA_To_Load_Typelib_References_By_Path_From_Untrusted_Intranet_Locations_User()
        {
            try
            {
                ...
                await securityBaseline.CreateNewProfile(
                                                        ...
                                                        "Allow VBA to load typelib references by path from untrusted intranet locations (User)", // Parent dropdown name
                                                        "Enabled",                        // Config 1 (CSV shows 'Enable' but should be 'Enabled')
                                                        "",                               // No child dropdown
                                                        "",                               // No child value
                                                        "Security Settings",              // Parent path
                                                        ""                                // Child path
                                                    );    
                _test?.Pass("Successfully created NEW Profile with Config 1: Enable");
            ...
            {
                var errorScreenshot = await ExtentReportHelper.CaptureScreenshot(Page, "Error_21790040");
