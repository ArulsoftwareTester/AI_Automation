...
{
    /// <summary>
    /// Data-Driven Tests for VMSync_TestData.json
    /// Executes tests in 3 parallel batches with separate browser contexts
    /// Add tests to TestData/VMSync_TestData.json - auto-discovered
    /// </summary>
    public abstract class VMSyncTestBase : SecurityBaseline
    {
        ...
        {
            Console.WriteLine($"Test Data - VMSync: {TestDataLoader.GetDataSummary()}");
            
            // Set Playwright timeout to 60 seconds for Login and VMSync operations
            if (Page != null)
            {
        ...
        protected async Task ExecuteVMSyncTest(string testCaseId)
        {
            ...
            Assert.That(testData, Is.Not.Null, $"Test case {testCaseId} not found in VMSync_TestData.json");
    ...
    #region VMSync Parallel Batches

    [TestFixture]
    public class VMSync_Batch1 : VMSyncTestBase
    {
    ...
    [TestFixture]
    public class VMSync_Batch2 : VMSyncTestBase
    {
        public static IEnumerable<string> GetTestCases()
        {
            var allTests = TestDataLoader.GetVMSyncData().TestCases
    ...
    [TestFixture]
    public class VMSync_Batch3 : VMSyncTestBase
    {
        public static IEnumerable<string> GetTestCases()
        {
            var allTests = TestDataLoader.GetVMSyncData().TestCases
