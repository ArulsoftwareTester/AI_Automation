# Script to run ALL Security Baseline for Windows 10 and later tests
# Configuration: 3 parallel workers, 20-minute per-test timeout, headless mode
# Generates Extent Report at the end
$ErrorActionPreference = "Continue"

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "Running Security Baseline for Windows 10 and later Tests" -ForegroundColor Cyan
Write-Host "Mode: Batches of 50 tests with 3 parallel workers (Headless)" -ForegroundColor Cyan
Write-Host "Per-Test Timeout: 20 minutes" -ForegroundColor Cyan
...
Write-Host "Using 3 parallel workers (reduced from 7 to prevent portal overload)" -ForegroundColor Yellow
Write-Host "Per-test timeout: 20 minutes (increased from 15 minutes)" -ForegroundColor Yellow
Write-Host "Mode: HEADLESS`n" -ForegroundColor Yellow

# Build the project first
Write-Host "Building project..." -ForegroundColor Yellow
$buildResult = dotnet build "IntuneCanaryTests\IntuneCanaryTests.csproj" --configuration Debug 2>&1 | Out-String
if ($LASTEXITCODE -ne 0) {
    Write-Host "Build failed! Attempting to run tests anyway..." -ForegroundColor Red
...
for ($batchNum = 1; $batchNum -le $totalBatches; $batchNum++) {
    ...
    Write-Host "Running $($batchTests.Count) tests with 3 parallel workers..." -ForegroundColor Cyan
    Write-Host "========================================" -ForegroundColor Cyan
    ...
    # Run batch with 3 parallel workers, 20-min timeout, headless mode
    $result = dotnet test "IntuneCanaryTests\IntuneCanaryTests.csproj" `
        --settings "security_baseline_3workers.runsettings" `
        --filter $filter `
        --logger:"console;verbosity=minimal" `
        --no-build `
        -- NUnit.NumberOfTestWorkers=3 NUnit.DefaultTimeout=1200000 `
        2>&1 | Out-String
    ...
    elseif ($result -match "Failed!\s+-\s+Failed:\s+(\d+),\s+Passed:\s+(\d+),\s+Skipped:\s+(\d+)") {
        ...
        Write-Host ($outputLines | Select-Object -Last 30 | Out-String) -ForegroundColor Gray
    }
...
Write-Host "Workers per Batch: 3 (Reduced from 7 to prevent portal overload)" -ForegroundColor White
Write-Host "Per-Test Timeout: 20 minutes (Increased from 15 minutes)" -ForegroundColor White
...
if ($allReports.Count -gt 0) {
    ...
    # Check for any recent reports
    $extentReportsPath = "ExtentReports"
    if (Test-Path $extentReportsPath) {
        $recentReports = Get-ChildItem -Path $extentReportsPath -Filter "TestReport_*.html" -File | 
            Where-Object { $_.LastWriteTime -ge $overallStartTime } |
            Sort-Object LastWriteTime -Descending
        if ($recentReports) {
            Write-Host "Total Reports: $($recentReports.Count)" -ForegroundColor Green
